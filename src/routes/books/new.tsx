  ...
  async function handleSave() {
    ...
    try {
      const book = await createBook({
        data: { name: name.trim(), description: description.trim(), songIds },
      })
      navigate({ to: '/books/$bookId' as '/', params: { bookId: book.id } as any })
    } finally {
      setSaving(false)
    }
  }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-white">New Book</h1>
          </div>
          <button
            onClick={handleSave}
